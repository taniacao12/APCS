...
public class StatPrinter {
	...
	for (int index = 1; index < _frequency.size() - 1; index ++) {
	    if (isLocalMode(_frequency.get(index)) == true) {
		modes.add(_frequency.get(index));
	    }
	}
	return modes;
    }
	...
	for (int index = 0; index < _frequency.size(); index ++) {
	    String bar = "";
	    for (int count = 1; count <= _frequency.get(index); count ++) {
		bar += "**";
	    }
